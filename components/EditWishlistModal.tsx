...
export default function EditWishlistModal({ isOpen, item, onClose, onItemUpdated }: EditWishlistModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateWishlist(item.id, title.trim(), author.trim());
      onItemUpdated();
      onClose();
    } catch (err) {
      setError('Erreur lors de la modification du souhait. Veuillez réessayer.');
      console.error(err);
    } finally {
